...
angular.module('weatherApp').service('weatherService', function ($http, $q) {
    ...
    this.getWeather = function (lat, lon) {
        var def = $q.defer();
        $http.get('http://api.openweathermap.org/data/2.5/weather?lat=' + lat + '&lon=' + lon + '&units=imperial&appid=48a8f7e3f9111f2ae148e9d7d078c129').then(function (resp) {
    ...
    this.getForecast = function (city) {
        var def = $q.defer();
        $http.get('http://api.openweathermap.org/data/2.5/forecast?q=' + city + ',us&units=imperial&appid=48a8f7e3f9111f2ae148e9d7d078c129').then(function (resp) {
    ...
    function buildDateArray(arr) {
        ...
        for (var i = 0; i < arr.length; i++) {
            ...
            if (time == '21' || i == arr.length - 1) {
                ...
                var day = new DailyInfo(moment(date).format('MMM D'), moment(date).format('ddd'), Math.round(Math.max.apply(Math, _toConsumableArray(temps))), Math.round(Math.min.apply(Math, _toConsumableArray(temps))), windAvg, Math.round(humidAvg), getImage(dailyDesc));
